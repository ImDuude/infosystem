<template>
    <div class="container">
        ...
        <div class="posts-container">
            <div class="post"
                 v-for="(post, index) in Maschines"
                 v-bind:item="post"
                 v-bind:index="index"
                 v-bind:key="post._id"
                 v-on:dblclick="delMaschine(post._id)">
                <div>
...
<script>
import MaschineService from '../MaschineService';


export default {
  name: 'MaschineComponent',
  data() {
    return {
      Maschines: [],
      error: '',
  ...
  async created() {
    try {
       this.Maschines = await MaschineService.getMaschine();
    } catch(err) {
  ...
  methods: {
    // async createMaschine() {
    //   await MaschineService.insertMaschine(this.name, this.gewicht, this.laenge, this.breite, this.hoehe);
    //   this.Maschines = await MaschineService.getMaschine();
    //   },
    async showhide() {
    ...
    async delMaschine(id) {
      await MaschineService.deleteMaschine(id);
      this.Maschines = await MaschineService.getMaschine();
    }
  }
};
